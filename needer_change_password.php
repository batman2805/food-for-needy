...
<button type="submit" class="button button-contactForm boxed-btn" name="submit">update</button>
               ...
               <?php
                                                if(isset($_POST['submit']))
                                                {
                                                    ...
                                                    $qry="select * from needer_detail where  needer_password='".$_POST['old_password']."' and needer_id='".$id."'";
                                                    $result=mysqli_query($conn,$qry) or die(mysqli_error());
                                                    $count=mysqli_num_rows($result);
                                                    if($count > 0)
                                                    {
                                                        $qry="update needer_detail set needer_password='".$_POST['new_password']."' where needer_id='$id'";
