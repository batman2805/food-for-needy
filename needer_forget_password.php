...
<label class="col-lg-3 col-form-label" for="val-username">Needer Email Id
 <span class="text-danger"></span>
   </label>
<div class="row">
<div class="col-6">
<div class="form-group">
<input class="form-control valid" name="needer_email_id" id="name" type="text" placeholder="Enter your email id" required="true">
</div>
</div>
...
<button type="submit" class="button button-contactForm boxed-btn" name="submit">Submit</button>
               ...
               <?php
                                                if(isset($_POST['submit']))
                                                {
                                                    ...
                                                    $qry="select * from needer_detail where needer_email_id='".$_POST['needer_email_id']."'";
                                                    $result=mysqli_query($conn,$qry) or die(mysqli_error());
                                                    $count=mysqli_num_rows($result);
                                                    $row=mysqli_fetch_array($result);
                                                    if($count > 0)
                                                    {    
                                                        $send_email=$_POST['needer_email_id'];
                                                        $send_password= "Your Password is : " . $row['needer_password'];
